...
- Войти в Xcode для существующей команды H23ADBVT65. App Store Connect повторно проверен непосредственно перед архивированием: последняя сборка — 1.0.0 (4), номер 5 свободен.
- Загрузить подготовленный архив, дождаться обработки, проверить установку через внутреннюю группу, добавить сборку в существующую внешнюю группу Equinox и пройти Beta App Review, если потребуется.
...
Новый архив **1.0.0 (5)**: `build/Archives/equinox-public-beta-2026-09-26-round-two.xcarchive`. Проверены arm64 приложения и EquinoxKit, минимальная macOS 26, ресурсы и локализация, подпись `--deep --strict`, Sandbox, calendar entitlements и отсутствие debugger entitlement. Архивы первого прохода устарели и не должны загружаться. Манифест с коммитом и контрольными суммами, тексты «Что тестировать», настройки экспорта и логи находятся в `build/TestFlight/2026-09-26-round-two`.

Попытка отправки нового архива 26 сентября в 08:16 MSK снова завершилась `Failed to Use Accounts`: Xcode требует App Store Connect access для H23ADBVT65. Загрузка на сервер не началась. В Xcode через интерфейс подтверждён пустой список Apple Accounts, открыто окно входа для пользователя. Transporter на Mac не установлен.
