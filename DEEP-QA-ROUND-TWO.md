...
Повторный проход начат с `97e5d87`, после первого аудита и изменения ручки боковой панели. Найдены и исправлены пять дополнительных проблем. Автоматические проверки, сборка Release и повторный живой прогон выполнены. Сборка 1.0.0 (5) загружена и обработана App Store Connect. Обновление и запуск через внутреннюю группу проверены. Apple одобрила Beta App Review; сборка доступна во внешней группе Equinox, итоговый статус — «Тестируется». Публичная ссылка сохранена.
...
- В **08:47 MSK 26 сентября** подтверждено одобрение сборки 5 во внешней группе, в **08:48 MSK** — итоговый статус **«Тестируется»** с обеими группами Equinox. Общедоступная ссылка включена и открывает страницу Equinox Calendar: [TestFlight](https://testflight.apple.com/join/N2t7wjvr). **Публичная бета 1.0.0 (5) выпущена.**
- Публичная доступность подтверждена состоянием App Store Connect и работающей страницей приглашения; установка и запуск — через внутреннюю группу на этом Mac. Повторное вступление по публичному приглашению под отдельной внешней учётной записью не проверялось. Попытка автоматического перехода из Chrome в TestFlight прервана управлением браузером, поэтому успешный переход по custom URL не заявляется.

Подтверждение выпуска: [одобрение внешней группы](docs/qa/2026-09-26-round-two/testflight-external-approved.png), [сборка 5 тестируется в обеих группах](docs/qa/2026-09-26-round-two/testflight-public-testing.png).
