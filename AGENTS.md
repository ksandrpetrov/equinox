...
| Tool | Назначение |
|------|------------|
| `get_calendar_access_status` | Проверка TCC для bridge |
| `request_calendar_access` | Запрос разрешения (системный диалог) |
| `list_calendars` | Список календарей |
| `list_events` | События за диапазон |
| `get_event` | Одно событие по ID |
| `create_event` | Создание |
| `update_event` | Обновление |
| `delete_event` | Удаление |
| `analyze_schedule` | Статистика занятости |
| `find_conflicts` | Пересечения событий |
| `find_free_time` | Свободные слоты |
